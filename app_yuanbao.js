// 全局变量
const API_URL = "http://www.sdhgjs.net:5027/face/getUserInfo"; // 照片查询接口
        ...
        html += `
            <div class="result-item ${!hasPhoto ? 'no-photo' : ''}">
                ...
                <div class="result-info">
                    ...
                    <p><span class="label">电话</span>${item.phone}</p>
...
function downloadSinglePhoto(name, account, phone, dept, imgStr) {
    try {
        ...
        // 使用格式：名字_账号_电话_部门
        const fileName = `${safeName}_${account}_${phone}_${safeDept}.jpg`;
...
function downloadAllPhotos() {
    ...
    const zip = new JSZip();
    const photoFolder = zip.folder('鲁化用户照片');
    ...
    hasPhotos.forEach(item => {
        ...
        // 使用格式：名字_账号_电话_部门
        const fileName = `${safeName}_${item.account}_${item.phone}_${safeDept}.jpg`;
